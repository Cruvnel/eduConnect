...
public class AgendaController : ControllerBase
{
    ...
    [HttpPost]
    public async Task<IActionResult> Criar([FromBody] AgendaCreateDto request)
    {
        try
        {
            ...
            return CreatedAtAction(nameof(ObterPorId), new { id = response.AgendaEventoId }, response);
        }
    ...
    public async Task<IActionResult> ObterPorId(int id)
    {
        try
        {
            ...
            if (response is null)
                return NotFound(new { mensagem = "Evento de agenda não encontrado." });
